 ...
 *
 * File: com.tencent.ibg.joox.data.settings/SettingsRepository
 * Created: 2025/8/8
 */
...
class SettingsRepository(private val context: Context) {
    ...
    val carLyricEnabledFlow: Flow<Boolean> =
        settingFlow(AutoSettingsSchema.lyrics.carLyricEnabled)
    ...
    suspend fun setCarLyricEnabled(enabled: Boolean) {
        if (enabled) {
            // 车载整首/单行歌词依赖蓝牙歌词数据源，开启时自动确保其可用
            setExternalBluetoothLyricsEnabled(true)
            setExternalBluetoothTranslationEnabled(true)
        }
        setSetting(AutoSettingsSchema.lyrics.carLyricEnabled, enabled)
    }
