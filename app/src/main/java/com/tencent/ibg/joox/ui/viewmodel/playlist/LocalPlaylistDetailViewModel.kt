 ...
 *
 * File: com.tencent.ibg.joox.ui.viewmodel.playlist/LocalPlaylistDetailViewModel
 * Updated: 2026/3/23
 */
...
import com.tencent.ibg.joox.data.local.playlist.LocalPlaylistDeleteResult
...
class LocalPlaylistDetailViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun metadataRichnessScore(song: SongItem): Int {
        ...
        if (song.artist.isMeaningfulMetadata(app.getString(com.tencent.ibg.joox.R.string.music_unknown_artist))) {
            score += 2
        }
    ...
    private fun hasMeaningfulScanMetadata(song: SongItem): Boolean {
        val unknownArtist = app.getString(com.tencent.ibg.joox.R.string.music_unknown_artist)
        val fileTitle = song.localFileName
            ?.substringBeforeLast('.', song.localFileName)
            ?.trim()
            .orEmpty()
