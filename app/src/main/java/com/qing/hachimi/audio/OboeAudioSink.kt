...
class OboeAudioSink(
    ...
    /** 路由可变：USB 独占/固定输出切换逐曲生效（由 HachimiAudioBridge 决策驱动） */
    @Volatile
    private var audioApi: Int = audioApi

    @Volatile
    private var exclusive: Boolean = exclusive

    @Volatile
    private var deviceId: Int = deviceId

    /** 每次重配前刷新路由（音频 API / 是否独占 / 目标设备） */
    fun updateRouting(audioApi: Int, exclusive: Boolean, deviceId: Int) {
